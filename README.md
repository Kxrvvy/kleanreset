...
A booking and quote-request website built for **Kleanreset**, a residential and commercial cleaning business in Edmonton, Alberta. Live at **[kleanreset.vercel.app](https://kleanreset.vercel.app)**.
...
The "from" address is swappable via a single environment variable — the pipeline runs today from Resend's sandbox address for testing, and moves to a verified custom domain (with SPF/DKIM/DMARC) without touching any code.
...
The project runs on three environment variables, set in Vercel's project settings (and locally in `.env.local`, untracked):

```
RESEND_API_KEY=       # from resend.com
RESEND_FROM_EMAIL=    # sandbox address during development, verified domain in production
BUSINESS_EMAIL=       # where booking/contact notifications are delivered
```
...
The site is live and functional end to end: all four marketing pages, the full booking flow with live pricing, the contact form, and the email pipeline are working in production. Outstanding: a verified custom domain (currently on a Vercel subdomain and Resend's sandbox sender), and legal review of the Terms and Privacy placeholder pages before they're treated as final.
